        ...
        <div className="min-h-screen bg-base-200 p-4 transition-colors duration-300">
            <header className="mb-6 flex justify-between items-center">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="text-3xl font-bold text-base-content">{t('kitchen.title')}</h1>
                </div>
                <div className="flex gap-4 items-center">
                    <div className="flex gap-2">
                        <div className="badge badge-lg badge-primary">{t('kitchen.pending')}: {activeOrders.filter(o => o.status === 'Pending').length}</div>
                        <div className="badge badge-lg badge-info">{t('kitchen.cooking')}: {activeOrders.filter(o => o.status === 'Cooking').length}</div>
                    </div>
                    <LanguageSwitcher />
                    <ThemeToggle />
                </div>
